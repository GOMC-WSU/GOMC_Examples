...
Hybrid MD/MC simulation of liquid ethane using the CHARMM force field in the grand canonical ensemble with a chemical potential of -3500 K and a temperature of 220 K.
...
- Box 0 (system): `required_data/input/ethane_GCMC_N1000_box0.pdb` and `.psf`
- Box 1 (reservoir): `required_data/input/ethane_GCMC_N1000_box1.pdb` and `.psf`
...
- `GCMC_ChemPot_or_Fugacity_dict`: Chemical potential (K) or fugacity (bar) per residue type; this example uses `{"ETH": -3500.0}`.

### Compute & Binary Paths
- `no_core_box_0`: CPU cores allocated to the Box 0 NAMD run (`4`).
- `starting_ff_file_list_gomc`: GOMC force field parameter files (`["required_data/input/ETH_FF_GOMC.inp"]`).
- `starting_ff_file_list_namd`: NAMD force field parameter files (`["required_data/input/ETH_FF_NAMD.inp"]`).
...
- **Expected liquid density**: 527.7774 kg/m³ (220 K)
