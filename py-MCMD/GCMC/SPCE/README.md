# SPC/E Water GCMC Example

Hybrid MD/MC simulation of SPC/E water in the grand canonical ensemble with a
chemical potential of -4210 K and a temperature of 510 K.
...
└── required_data/
    ...
    ├── input/                   # Force fields and coordinate/topology (.pdb/.psf) files
...
Box 0 holds the SPC/E water system; Box 1 is the ideal-gas reservoir that
supplies and accepts molecules during the insertion and deletion moves. One cycle
is one molecular dynamics run followed by one Monte Carlo run, and
...
- Box 0 (system): `required_data/input/setup_box_0.pdb` and `setup_box_0.psf`
- Box 1 (reservoir): `required_data/input/setup_box_1.pdb` and `setup_box_1.psf`
...
- `GCMC_ChemPot_or_Fugacity_dict`: Chemical potential (K) or fugacity (bar) per residue type; this example uses `{"SPCE": -4210}`.

### Compute & Binary Paths
- `no_core_box_0`: CPU cores allocated to the Box 0 NAMD run (`4`).
- `starting_ff_file_list_gomc`: GOMC force field parameter files (`["required_data/input/GOMC_SPCE_FF.inp"]`).
- `starting_ff_file_list_namd`: NAMD force field parameter files (`["required_data/input/NAMD_SPCE_FF.inp"]`).
