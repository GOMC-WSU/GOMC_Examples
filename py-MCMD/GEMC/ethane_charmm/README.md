...
Gibbs ensemble at a temperature of 250 K, giving the vapor and liquid densities at
...
Molecular dynamics is performed in the canonical (NVT) ensemble on both boxes and
is used to sample configurational and conformational degrees of freedom.
...
topology (`.psf`) files for both boxes. The starting configurations are already
equilibrated, so `namd_minimize_mult_scalar` is `0` (no energy minimization):
- Box 0 (liquid): `required_data/input/ETH_equil_BOX_0_restart.pdb` and `.psf` (707 ethane molecules)
- Box 1 (vapor): `required_data/input/ETH_equil_BOX_1_restart.pdb` and `.psf` (293 ethane molecules)

## Input Parameters (`user_input_NAMD_GOMC.json`)

### Simulation Controls
- `simulation_type`: Ensemble type (`"GEMC"`).
- `simulation_temp_k`: Simulation temperature in Kelvin; this example runs at `250.0`.
- `only_use_box_0_for_namd_for_gemc`: Run molecular dynamics on Box 0 only (`true`) or on both boxes (`false`); this example uses `false`.
- `namd_simulation_order`: Order for the two NAMD runs when both boxes use molecular dynamics (`"series"` or `"parallel"`); this example uses `"parallel"`.
...
- `no_core_box_1`: CPU cores allocated to the Box 1 NAMD run (`4`; nonzero because this example runs molecular dynamics on both boxes).
...
At 250 K, the coexisting densities are:
- **Expected liquid density**: 446.4655 kg/m³
- **Expected gas density**: 27.7492 kg/m³
