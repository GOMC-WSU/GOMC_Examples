...
Hybrid NAMD/GOMC simulation of vapor-liquid phase equilibrium for F1A-C1A in the Gibbs (GEMC) ensemble.
...
This example simulates vapor-liquid phase coexistence of a F1A-C1A system using two simulation boxes:
- **Box 0**: Liquid phase.
- **Box 1**: Vapor phase.
...
- `starting_ff_file_list_gomc`: Force field parameter files for GOMC (`["required_data/input/TraPPE_FF.inp"]`).
- `starting_ff_file_list_namd`: Force field parameter files for NAMD (`["required_data/input/TraPPE_FF.inp"]`).
